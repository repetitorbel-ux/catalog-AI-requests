class CatalogApp {
    ...
    setupCatalogEventListeners() {
        document.querySelectorAll('.category').forEach(p => {
            p.addEventListener('click', (e) => this.loadSubcategories(e.target.dataset.name));
        });
        document.querySelectorAll('.subcategory').forEach(p => {
            p.addEventListener('click', (e) => this.loadEntries(e.target.dataset.category, e.target.dataset.name));
        });
        document.querySelectorAll('.rename-link').forEach(a => {
            a.addEventListener('click', (e) => {
                e.stopPropagation();
                this.showRenamePrompt(a.dataset.type, a.dataset.id, a.dataset.name);
            });
        });
    }
    ...
    loadEntries(categoryName, subcategoryName) {
        this.toggleSpinner(true);
        document.querySelectorAll('.subcategory').forEach(el => el.classList.remove('selected'));
        document.querySelector(`p[data-name='${subcategoryName}'][data-category='${categoryName}']`).classList.add('selected');
